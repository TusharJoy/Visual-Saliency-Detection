...
//	imshow("Diff L channel", spl_diff[0]);  //L
	...
	for (int y = 0; y < (int)Imp_prime.rows; ++y) {
		for (int x = 0; x < (int)Imp_prime.cols; ++x) {
			int lbl = labels.at<int>(y, x);
			int val =(int) Imp_prime.at<uchar>(y, x);
			//cout << val << endl;
			num_pixels[lbl] += val;
			pixels_counter[lbl]++;
			//cout <<"LBL = " <<lbl<<" Pixels value ="<<num_pixels[lbl]<<" = "<<pixels_counter[lbl] << endl;
		}
	}
	
	for (int i = 0; i < centers.size(); i++)
	{
		//cout <<"LABEL =  "<<num_pixels[i] << "Number of pixels =  " << pixels_counter[i] << endl;
...
int main()
{
	...
	for (size_t i = 0; i<fn.size(); i++)
	{
		...
		*/
		...
		{
			float sal = 0.8* boundary_sal[i]  + 0.1* global_sal[i] + color_Saliency[i] * 0.1;
